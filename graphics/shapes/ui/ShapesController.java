...
public class ShapesController extends Controller {
  ...
  public void mouseDragged(MouseEvent evt) {
    if (this.state == 0) {
      ...
      if (this.prev == null) {
        ...
        int dx = now.x - this.prev.x;
        int dy = now.y - this.prev.y;
        this.prev = now;
  ...
  public void up() {
    ...
    while (iterator.hasNext()) {
      shape = iterator.next();
      if (
        ((ColorAttributes) shape.getAttributes(id)).filledColor.equals(
            Color.red
          ) &&
        shape.getBounds().y > 0
      ) {
  ...
  public void down() {
    ...
    while (iterator.hasNext()) {
      ...
      if (
        ...
        shape.getBounds().y + shape.getBounds().height < 300
      ) {
        shape.setLoc(new Point(shape.getLoc().x, shape.getLoc().y + 4));
      }
      if (
  ...
  public void right() {
    ...
    while (iterator.hasNext()) {
      ...
      if (
        ...
        shape.getBounds().x + shape.getBounds().width < 300
      ) {
    ...
    if (r != null) {
      ...
      while (iterator.hasNext()) {
        ...
        if (
          ...
          if (
            shape.getBounds().contains(new Point(r.x, r.y)) ||
            ...
            if (
              shape.getBounds().x + shape.getBounds().width <= r.x + 3 &&
              shape.getBounds().x + shape.getBounds().width >= r.x - 3
            ) {
              score += 1;
              if (score > highScore) {
